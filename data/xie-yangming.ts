export type XieTone = '克己' | '定心' | '进取' | '豁达'
export type XieLength = '四句' | '六句'
...
【文风边界（必须遵守）】
1) 严格使用短章句：只允许“四句”或“六句”。
2) 每句应有明显节奏，优先 4-6 或 6-4 字段落感（可有少量浮动，但整体整齐）。
...
export const buildXieYangmingUserPrompt = (input: XieInput) => {
  const length = input.length ?? '四句'
  const tone = input.tone ?? '定心'
